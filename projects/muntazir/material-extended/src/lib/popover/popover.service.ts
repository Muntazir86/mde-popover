...
export class PopoverService {
  ...
  getFallbacks(
    ...
    // cover the anchor
    const possibleHorizontalAlignments: MdePopoverPositionX[] = horizontalOverlapAllowed
      ? ['before', 'start', 'center', 'end', 'after']
      : ['before', 'after'];
    const possibleVerticalAlignments: MdePopoverPositionY[] = verticalOverlapAllowed
  ...
  getPosition(xPosition: MdePopoverPositionX, yPosition: MdePopoverPositionY) {
    const { originX, overlayX } = this.getHorizontalConnectionPosPair(xPosition);
  ...
  getHorizontalPopoverAlignment(h: HorizontalConnectionPos): MdePopoverPositionX {
    if (h === 'start') {
      return 'after';
    }
  ...
  getNonOverlapPosition(posx: MdePopoverPositionX, posy: MdePopoverPositionY): { PositionX: MdePopoverPositionX, PositionY: MdePopoverPositionY } {
    let positionX = posx;
    let positionY = posy;
    switch (posx) {
      case 'start':
