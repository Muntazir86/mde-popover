...
export class MdePopoverTrigger implements AfterViewInit, OnDestroy { // tslint:disable-line:directive-class-suffix
  ...
  @Input('mdePopoverArrowOffsetY') arrowOffsetY: number;
  ...
  */
  private _subscribeToPositions(position: FlexibleConnectedPositionStrategy): void {
    this._positionSubscription = position.positionChanges.subscribe(change => {
      const posisionX: MdePopoverPositionX = this.popoverService.getHorizontalPopoverAlignment(change.connectionPair.overlayX);
      let posisionY: MdePopoverPositionY = this.popoverService.getVerticalPopoverAlignment(change.connectionPair.overlayY);
